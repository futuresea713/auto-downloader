...
def get_clients(page, search_key):
    ...
    clients = []
    #link = 'https://assistingh.theranest.com/api/clients/getListing?searchKey={}&staffId=&status=&serviceTypeId=&page={}'.format(search_key, page)
    link = 'https://assistingh.theranest.com/api/clients/getListing?page=' + str(page)
...
def down_file(client_folder,url):
...
client_sel = []
def get_client_sel(page_num):
    url = "https://assistingh.theranest.com/clients/listing/staff-all/active/all-services/page-" + str(page_num)
    driver.get(url)

    clients = []
    try:
        clients_grid = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, "clients_grid")))
        clients_ = clients_grid.find_elements_by_css_selector("div.desktop")

        for clints in clients_:
            cliet = clints.find_elements_by_css_selector("a.btn.btn-small")
            for idx, client in enumerate(cliet):
                if idx == 1:
                    client_sel.append(client.get_attribute("href"))
                    continue
        return clients
    except:
        print("get clients error")




def check_exists_by(tr):
    try:
        tr.find_element_by_css_selector("button.btn.btn-small.removeNote")
...
def download_file_sel(download_file_sel,url):
    client_folder2 = outputs + '/' + client['FullName'] + "/Open Case Notes"
    client_folder1 = outputs + '/' + client['FullName'] + "/Closed Case Notes"
    driver.get(url)
    time.sleep(2)

    try:
        table = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "table.table.table-striped.table-bordered.table-radmin")))
        alltrtags = table.find_elements_by_tag_name("tr")
        for i,tr in enumerate(alltrtags):
            if i != 0:
                exist = check_exists_by(tr)
                get_file = tr.find_element_by_css_selector("a.btn.btn-small.skip-readonly.print-note")
                downlink = get_file.get_attribute("href")
                if exist == True:
                    down_file(client_folder2,downlink)
                else:
                    down_file(client_folder1,downlink)
    except:
        pass
...
def get_allcase(url):
    ...
    links = []
    try:
        all_tab = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "ul.nav.nav-tabs.grey-nav-tabs")))
        allcase = all_tab.find_element_by_tag_name("a")
        caseurl = allcase.get_attribute("href")
        driver.get(caseurl)
        time.sleep(1)
        all_body = driver.find_elements_by_css_selector("table.table.table-striped.table-bordered.table-radmin")
        for id,body in enumerate(all_body):
            if id == 1:
                td = body.find_element_by_tag_name("td")
                txt = td.text
                if txt != "No closed cases found":
                    with open('urls(closed).txt', 'a') as the_file:
                        the_file.write(caseurl + '\n')
                break
        all_buttons = driver.find_elements_by_css_selector("a.btn.btn-small")
...
if __name__ == "__main__":
    ...
    page = input("typing page number:")
    page_num = int(page)
    while page_num > 0:
        _clients = get_clients(page_num, search_key)
        get_client_sel(page_num)
        if len(_clients) == 0:
            break
    ...
    for ic,client in enumerate(clients):
        ...
        client_folder2 = outputs + '/' + client['FullName'] + "/Open Case Notes"
        client_folder1 = outputs + '/' + client['FullName'] + "/Closed Case Notes"
        if not os.path.exists(client_folder2):
            try:
                os.mkdir(client_folder2)
            except Exception as e:
                print('Client Folder Exception:', str(e))
        if not os.path.exists(client_folder1):
            try:
                os.mkdir(client_folder1)
            except Exception as e:
                print('Client Folder Exception:', str(e))
        allcase = get_allcase(client_sel[ic])
        if len(allcase) != 0:
            for url in allcase:
                download_file_sel(client_folder,url)
        client_id = client['Id']
